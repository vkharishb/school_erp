...
export default function AccountSecurityPage(){
  ...
  return <div className="max-w-2xl space-y-6"><div><h1 className="text-2xl font-bold">Account Security</h1><p className="text-gray-500 mt-1">Change your own password. All active sessions are revoked after a successful change.</p></div>{error&&<div className="rounded-lg bg-red-50 border border-red-200 text-red-700 px-4 py-3 text-sm">{error}</div>}<form className="card space-y-4" onSubmit={submit}><Field id="current-password" label="Current password" value={currentPassword} onChange={setCurrentPassword} autoComplete="current-password"/><Field id="new-password" label="New password" value={newPassword} onChange={setNewPassword} autoComplete="new-password" minLength={10}/><Field id="confirm-new-password" label="Confirm new password" value={confirm} onChange={setConfirm} autoComplete="new-password" minLength={10}/><p className="text-xs text-gray-500">Use 10–128 characters and at least three character classes: uppercase, lowercase, number and symbol. Common passwords are blocked.</p><button className="btn-primary">Change password & sign out</button></form></div>
}
function Field({id,label,value,onChange,autoComplete,minLength}:{id:string;label:string;value:string;onChange:(v:string)=>void;autoComplete:string;minLength?:number}){return <label htmlFor={id}><span className="label">{label}</span><PasswordInput id={id} value={value} onChange={onChange} autoComplete={autoComplete} minLength={minLength}/></label>}
