...
export default function BulkImportsPage(){
  ...
  return <div className="space-y-6">
    ...
    <section className="card"><div className="flex flex-wrap items-center justify-between gap-4"><p className="text-sm text-gray-500">Imports apply to the current School.</p>{canManageClassSection&&<Link className="btn-secondary" to={`/schools/${schoolId}/classes-sections`}>Create / Manage Classes & Sections</Link>}</div></section>
    ...
    {canClassSectionBulk&&<BulkImportPanel
      ...
      contextMessage="School setup is loading. The template can be downloaded now."
    ...
    {canStudent&&<BulkImportPanel
      ...
      description="Download the approved 20-field template for this School. Each student row contains its own Academic Year, Class and Section, and the ERP validates those values before import. SIS Number and Student Status are generated automatically. Review the preview before confirming the import."
      contextReady={!!campusId}
      requireContextBeforeFile
      requireContextBeforeTemplate
      contextMessage="School setup must be ready before downloading the School-specific ERP Reference sheet."
