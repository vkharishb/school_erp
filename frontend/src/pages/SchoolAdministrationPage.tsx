...
export default function SchoolAdministrationPage(){
 ...
 const items=[
  ...(canAccessSchoolDirectory(user)?[{title:"Society / Trust",desc:"Organization details, annual correction and School capacity.",to:"/organizations",icon:Building2,show:hasPermission(user,"organization.view")},{title:"Schools",desc:"Create and maintain the complete School profile.",to:"/schools",icon:School,show:true}]:[]),
  ...
  {title:"Roles & Permissions",desc:"Platform role and permission matrix. Platform Owner only.",to:schoolTarget("roles"),icon:ShieldCheck,show:!!user?.is_superuser},
  {title:"Modules",desc:"Review subscription modules for a selected School. Platform Owner only.",to:schoolTarget("modules"),icon:Boxes,show:!!user?.is_superuser},
  ...
  {title:"Classes & Sections",desc:"Create School classes and sections used by enrollment, fees and marks.",to:schoolTarget("classes-sections"),icon:GraduationCap,show:hasPermission(user,"academic_class.manage")||hasPermission(user,"school.config.view")},
  {title:"Subjects",desc:"Create and maintain School subject masters.",to:schoolTarget("subjects"),icon:BookOpen,show:hasPermission(user,"subject.manage")||hasPermission(user,"school.config.view")},
