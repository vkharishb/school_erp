...
export default function TeachersPage() {
  ...
  const [teachers, setTeachers] = useState<Teacher[]>([]),[search,setSearch]=useState(""),[error,setError]=useState(""),[message,setMessage]=useState("");
  const [form,setForm]=useState({employee_code:"",first_name:"",last_name:"",email:"",phone:"",designation:"",qualification:"",joining_date:""});
  const load=async()=>{try{setTeachers(await teacherApi.list(schoolId,search||undefined));}catch(e){setError(apiErrorMessage(e,"Failed to load teachers"));}};
